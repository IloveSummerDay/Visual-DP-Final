...
const props = defineProps(['title', 'content', 'duration'])
onMounted(() => {
...
<template>
  <div class="flex justify-evenly items-center w-[100%] mt-4 mb-4">
    ...
    <div
      :class="`content p-2 w-[60%] rounded-lg flex bg-white cursor-default`"
      ref="ani"
      @mouseenter="handleExplainOpen"
    >
      <span
        :class="`flex items-center p-2 ${props.content.length > 50 ? 'indent-[2rem]' : ''}`"
